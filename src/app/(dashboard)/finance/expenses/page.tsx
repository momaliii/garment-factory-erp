...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState({ categoryId: "", amount: "", description: "", date: new Date().toISOString().slice(0, 10), isRecurring: false });
  ...
  const save = async () => {
    ...
    try {
      ...
      setShowForm(false);
      setEditId(null);
      setForm({ categoryId: "", amount: "", description: "", date: new Date().toISOString().slice(0, 10), isRecurring: false });
      load();
  ...
  const startEdit = (e: ExpenseItem) => {
    setEditId(e.id);
    setForm({ categoryId: e.categoryId, amount: String(e.amount), description: e.description || "", date: e.date, isRecurring: e.isRecurring });
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ categoryId: "", amount: "", description: "", date: new Date().toISOString().slice(0, 10), isRecurring: false }); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-primary/90">
      ...
      {showForm && (
        <div className="bg-card border rounded-xl p-5 space-y-4">
          <h3 className="font-semibold">{editId ? "تعديل مصروف" : "إضافة مصروف"}</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground text-xs border-b">
                ...
                <th className="text-right py-2.5 px-4">متكرر</th>
            ...
            <tbody>
              {expenses.map((e) => (
                <tr key={e.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="py-2.5 px-4">{e.category.name}</td>
                  <td className="py-2.5 px-4 font-semibold text-red-600">{formatCurrency(e.amount)}</td>
                  <td className="py-2.5 px-4 text-muted-foreground">{e.description || "—"}</td>
                  <td className="py-2.5 px-4">{e.isRecurring ? "✓" : "—"}</td>
                  <td className="py-2.5 px-4">
                    <div className="flex gap-1">
                      <button onClick={() => startEdit(e)} className="p-1.5 rounded hover:bg-accent"><Edit className="w-3.5 h-3.5" /></button>
                      <button onClick={() => del(e.id)} className="p-1.5 rounded hover:bg-red-50 text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {expenses.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-muted-foreground">لا توجد مصروفات</td></tr>
              )}
            </tbody>
          </table>
        </div>
